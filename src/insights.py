...
def get_max_salary(path):
    ...
    jobs = read(path)
    salaries = [
        int(job["max_salary"]) for job in jobs
        if job["max_salary"].isnumeric()]
    bigest_salary = max(salaries)
    return bigest_salary
...
def get_min_salary(path):
    ...
    jobs = read(path)
    salaries = [
        int(job["min_salary"]) for job in jobs
        if job["min_salary"].isnumeric()]
    smallest_salary = min(salaries)
...
def matches_salary_range(job, salary):
    ...
    if (type(min_salary) != int or type(max_salary) != int
        ...
        raise ValueError
    match = min_salary <= salary <= max_salary
    return match
