...
def get_unique_job_types(path):
    ...
    """
    return list({item["job_type"] for item in read(path)})
...
def get_unique_industries(path):
    ...
    """
    return list(
        {item["industry"] for item in read(path) if item["industry"] not in ""}
    )
...
def get_min_salary(path):
    ...
    valid_salaries = [
        int(item["min_salary"])
        for item in read(path)
        if re.search("[0-9]+", item["min_salary"]) is not None
    ]
